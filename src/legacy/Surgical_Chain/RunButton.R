...
server <- function(input,output) {
      ...
      # Modularize the simulation and create a button that runs sim with observeEvent() 
      ...
      output$usage <- renderPlot({ 
            plot(get_mon_resources(sim()), metric = "usage", items = "queue") #use () for reactive values sim()
      })
