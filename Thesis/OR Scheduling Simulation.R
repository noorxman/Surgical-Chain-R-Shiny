...
t_signaler <- trajectory("signaler") %>%
   send(function() {
      #PRECONDITION: IF block is chosen the Types on the first day have to be different
      signals <- c()
...
### Test for multiple variable number of or 

for (i in 1:number_or){
   assign(paste0("function_OR",i), function(number){
...
for(i in 1:number_or) {
   assign(paste0("wait_for_patients_handler_OR",i),
          trajectory(paste0("wait_for_patients_handler_OR",i)) %>%
             
             trap(paste0("wait_for_patients_OR", i),
                  handler = trajectory() %>%
                     #Define variables as attributes to be used 
                     log_(function ()  paste0("Time now: ",as.character(now(hospital)))) %>%
                     # set_attribute("current_entrance", function() get_global(hospital,"Entrance")) %>%
